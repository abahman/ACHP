...
from PRESSURE import LossCoeffReturnBend, GET_PreAcc_GasCooler
from VOLUME import VolumeALL
from CORR import Circuit_DP_COND, ConvCoeffAir_CON, FinEffect_Schmidt, ConvCoeffInside, FricDP, Petterson
...
def CondTubeL_new(Gr,   #refigerant mass flux
    ...
    *********************************************************************/
    ...
    #P['q_w']=0 #initialize q with zero value
    #Iter=0;
    #h_in = HPi['H']
    #p_in = HPi['P']
    #HPm = HPi.copy()
    #TXPo = HPtoTXP(HPm,Ref) #initialize oulet
    #TXPm = HPtoTXP(HPm,Ref) #initialize mean (average)
    ...
    P['R_W']=log(P['Do']/(P['Do']-2.0*P['xp']))/(2.0*pi*P['K_T']*P['Ls']);
    ...
    phi = FinEffect_Schmidt(ho,P['K_F'],P['th'],P['y'],P['Do']);#fin efficiency, using Schmidt equation 
    ...
    def Get_Tw(T_w,Gr,P): 
        ...
        Ro = P['Ro'] + P['R_W']
        q_est = (1/Ro)*(T_w - Tai['T']);
        
        # Calculate heat transfered per unit mass of refrigerant.
        hi, fi, cp, rho = Petterson(TXPi['T'],T_w,Ref,Gr,P,P['Di']/P['Ls'],TXPi['P'],q_est/P['Api']);
        hi = P['hRefAdj']*hi
        Ri = 1/(hi*P['Api']);#refrigerant side thermal resistance
        
        T_wo = TXPi['T'] - q_est*Ri; #surface temperature on the outer wall
        
        R = (P['Ro']+P['R_W']+Ri);
        
        q_ntu = (TXPi['T']-Tai['T'])*CmineCrossFlow_dry(R,mr,ma,TXPi,Tai,Ref);
        
        #TXPo['T'] = TXPi['T'] - q_ntu/(mr*cp);
        #TXPm['T'] = 0.5*(TXPo['T']+TXPi['T']) #mean temperature
        #TXPm['P'] = TXPi['P']; #mean
        #HPm['H'] = HPi['H'] - q_ntu/mr;
        #TXPm=HPtoTXP(HPm,Ref)
        
        # Calculate heat transfered per unit mass of refrigerant.
        #hm, fm, cpm, rhom = Petterson(TXPm['T'],T_w,Ref,Gr,mr,P,P['Di']/(P['Ls']),TXPm['P'],q_ntu/P['Api']);
        #hm = P['hRefAdj']*hm
        #Rm = 1/(hm*P['Api']);#refrigerant side thermal resistance
        
        #q_target = (1/Rm) * (TXP['T']-T_w)
    ...
    #solve for wall temperature and heat rate
    P['T_w'] = brentq(Get_Tw,Tai['T'],TXPi['T'],args=(Gr,P,),xtol=1e-5,rtol=6e-8,maxiter=40)
    
    #recall the output from the brentq solver
    R = P['R']
    q = P['q_w']
    fi = P['fi'] #friction factor at average temperature
    rho = P['rho'] #density at average temperature 
    ...
    DP_FR=pow(Gr,2e0)*fi*P['Ls']/(2*P['Di'])/rho #[Pa] A.B. new function to calculate the fictional pressure drop, no matter single-phase or two-phase flow
...
#     if(TXPi['X']>0.05 and TXPi['X']<0.99):
    ...
    Preacc['P_IN']=TXPi['P'];
    Preacc['rho_IN']=rho;
    DP_ACC=brentq(GET_PreAcc_GasCooler,10000,-10000,args=(Ref, Preacc),xtol=1e-7,rtol=6e-8,maxiter=40)#B.S. two-phase acceleration pressure drop
...
#         P_out=TXPi['P'] - DP_FR*P['PRefAdj'];#assume single-phase flow has no accelerattion pressure drop
    ...
    v = 1/rho#VolumeALL(TXPi,Gr,P['Di'],-mr*q/P['Api'],Ref);        # seperate flow model
...
class StructCondClass():
    ...
    def __init__(self,filename,Ref):
        ...
        self.Velocity = df.CondStruc[6];           #total nominal air mass flow rate [m^3/s]
...
def Condenser(Ref,
    ...
    ***********************************************************************/
    ...
    if(condInit or NSeg>0):
        ...
        P['Nbranchs']=int(df.Condenser[12]);        # number of branchs in main section (A.B. number of circuits)
        P['Nmaintubes']=int(df.Condenser[13]);      # number of tubes in main section (A.B. tubes per circuit)
        P['Nsubtubes']=df.Condenser[14];            # number of tubes in subcooled section (A.B. we don't have subcooler)
        P['Ndeep']=df.Condenser[15];                # B.S. number of high rows (A.B. number of banks)
...
def ExerciseCondenser(Ta,v,Tr,p,MR): #AMB [C], velocity [m/s], T_ref [C], P_ref [MPa], mass flow [kg/s]
    ...
    AMB = Ta #[C]
    AMB_RH = 0.2 #[-] #A.B.
    MR = MR # kg/s, refrigerant mass flow rate
    GOA = v*0.281 # [m^3/s], outdoor air flow rate
    ...
    #inlet air state [K]
    Tai = {'T':C2K(AMB), 'P':AMB_RH}
    #volumteric flow rate (m^3/s)
    Goa = GOA;
    ...
    txpi = {'T':C2K(Tr),'X':1,'P':p*1000000} #initially assume outlet temperature equals to saturated vapor pressure (txpo_P)
    ...
    Qdot = mr*(hpi['H']-hpo['H']); #[Watts]- for all 4 sections of the heat exchanger
    ...
    #print("dP (kPa): ",((txpi['P']-txpo['P'])/1000))
    #print("mass (kg): ",m['m']) #charge
    #print("Tr_out (C): ",K2C(txpo['T']))
    #print("Qdot (kW): ",Qdot/1000)
    #print("UA (BTU/hr/F): ",UA)
    #print("Hin (kJ/kg): ",hpi['H']/1000)
    #print("Hout (kJ/kg): ",hpo['H']/1000)
    print(Qdot/1000, K2C(txpo['T']))
    return 0
...
if __name__=='__main__': 
    ...
    import pandas as pd
    #from CoolProp.CoolProp import PropsSI
    df = pd.read_excel('../ComponentTests/supercritical/Table.xlsx')
    for i in range(36):
        ExerciseCondenser(df['Air Inlet Air Temps'][i+1],df['Air Velocity'][i+1],df['Refrigerant Inlet Temp'][i+1],df['Refrigerant Inlet Pressure'][i+1],df['Refrigerant Flow Rate'][i+1])
